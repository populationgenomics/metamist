...
PROFILE_REQUESTS = (
    os.getenv('SM_PROFILE_REQUESTS', 'false').lower() in TRUTH_SET
)
PROFILE_REQUESTS_OUTPUT = os.getenv(
    'SM_PROFILE_REQUESTS_OUTPUT', 'text'
).lower()
IGNORE_GCP_CREDENTIALS_ERROR = (
    os.getenv('SM_IGNORE_GCP_CREDENTIALS_ERROR') in TRUTH_SET
)
MEMBERS_CACHE_LOCATION = os.getenv('SM_MEMBERS_CACHE_LOCATION')
METAMIST_GCP_PROJECT = os.getenv('METAMIST_GCP_PROJECT', 'sample-metadata')
...
SEQR_SLACK_NOTIFICATION_CHANNEL = os.getenv(
    'SM_SEQR_SLACK_NOTIFICATION_CHANNEL'
)

SAMPLE_PREFIX = os.getenv('SM_SAMPLEPREFIX', 'XPGLCL').upper()
SAMPLE_CHECKSUM_OFFSET = int(os.getenv('SM_SAMPLECHECKOFFSET', '2'))

SEQUENCING_GROUP_PREFIX = os.getenv(
    'SM_SEQUENCINGGROUPPREFIX', 'CPGLCL'
).upper()
SEQUENCING_GROUP_CHECKSUM_OFFSET = int(
    os.getenv('SM_SEQUENCINGGROUPCHECKOFFSET', '9')
)
...
COHORT_TEMPLATE_CHECKSUM_OFFSET = int(
    os.getenv('SM_COHORTTEMPLATECHECKOFFSET', '3')
)
