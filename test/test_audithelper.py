...
def gcs_list_blobs(bucket_name, prefix, delimiter):
    """Mocks the result of the gcs client list_blobs method"""
    transfer_blobs = ['transfer/data.cram',]
    transfer2_blobs = ['transfer/transfer2/data2.cram',]
    transfer3_blobs = ['transfer/transfer2/transfer3/data3.cram',]
    transfer4_blobs = ['transfer4/data4.cram',]
    seqdir_blobs = ['sequences/read_R1.fastq', 'sequences/read_R2.fastq.gz', 'sequences/mainfest.txt']
...
class TestAuditHelper(unittest.TestCase):
    ...
    def test_get_gcs_bucket_subdirs_to_search(
        self
    ):
        """
        ...
        """
        paths = ['gs://cpg-dataset-main-upload/transfer/data.cram',
        ...
        expected_result['cpg-dataset-main-upload'] = ['transfer/', 'transfer/transfer2/', 'transfer/transfer2/transfer3/', 'transfer4/',]
    ...
    def test_get_gcs_paths_for_subdir(
        self
    ):
        ...
        for subdir in [subdir1, subdir2, subdir3, subdir4]:
            for blob_name in gcs_list_blobs(
        ...
        expected_results = ['gs://cpg-dataset-main-upload/transfer/data.cram',
                            'gs://cpg-dataset-main-upload/transfer/transfer2/data2.cram',
                            'gs://cpg-dataset-main-upload/transfer/transfer2/transfer3/data3.cram',
                            'gs://cpg-dataset-main-upload/transfer4/data4.cram',]
    ...
    def test_find_sequence_files_in_gcs_bucket(
        self
    ) -> list[str]:
        ...
        with self.assertRaises(NameError):
            AuditHelper.find_sequence_files_in_gcs_bucket(self, bucket_name='cpg-dataset-main', file_extensions=('test',))

        for blob_name in gcs_list_blobs(bucket_name, prefix='sequences/', delimiter='/'):
            if blob_name.endswith(file_extensions):
                sequence_paths.append(f'gs://{bucket_name}/{blob_name}')
            continue

        expected_result = ['gs://cpg-dataset-main-upload/sequences/read_R1.fastq', 'gs://cpg-dataset-main-upload/sequences/read_R2.fastq.gz']
