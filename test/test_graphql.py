...
from models.models import SampleUpsertInternal, ParticipantUpsertInternal, SequencingGroupUpsertInternal, AssayUpsertInternal
  ...
  project(name: $project) {
    participants {
      ...
      samples {
        ...
        self.assertEqual(p.samples[0].sequencing_groups[0].id, sequencing_groups[0]['id'])
        assays = sequencing_groups[0]['assays']
        self.assertEqual(1, len(participants[0]['samples'][0]['sequencingGroups'][0]['assays']))
        self.assertEqual(p.samples[0].sequencing_groups[0].assays[0].id, assays[0]['id'])
