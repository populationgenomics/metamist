...
We take a metamist project...
Get all the samples for this project from Metamist
Get all the sequences for this project from Metamist

Check the bucket for all sequence data, and compare to the data in the metamist sequences
...
from sample_metadata.apis import ProjectApi, ParticipantApi, AnalysisApi, SampleApi, FamilyApi, SequenceApi
...
ALL_EXTENSIONS = ('.fq.gz', '.fastq.gz', '.fq', '.fastq', '.bam', '.cram', '.g.vcf.gz', '.vcf', '.vcf.gz')

ANALYSIS_TYPES = ['qc','joint-calling','gvcf','cram','custom','es-index','sv','web','analysis-runner']
...
def get_paths_for_subdir(bucket_name, subdirectory, file_type):
...
def get_files_in_buckets_subdirs(buckets_subdirs: list[tuple[str,str]], file_types: list[str]):
    """
...
def get_all_sequence_paths(bucket_name: str) -> list[str]:
...
def check_for_uningested_sequences(project: str, sequence_reads: list[dict]):
    """Compares the sequences in the project to the sequences in the main-upload bucket"""
    # Get all the paths to sequence data anywhere in the main-upload bucket
    bucket_name = f'cpg-{project}-main-upload'
    sequence_paths_in_bucket = get_all_sequence_paths(bucket_name)

    # Flatten all the sequence paths and sizes in metamist for this project into a single list
    ...
    sequence_paths_in_metamist = [path_size[0] for path_size in sequence_paths_sizes_in_metamist]
    uningested_paths = list(set(sequence_paths_in_bucket).difference(set(sequence_paths_in_metamist)))

    # Strip the metamist paths into just filenames
    sequence_files_sizes_in_metamist = {get_filename_from_path(path[0]):path[1] for path in sequence_paths_sizes_in_metamist}
    sequence_files_paths_in_metamist = {get_filename_from_path(path[0]):path[0] for path in sequence_paths_sizes_in_metamist}
    ...
    for path in uningested_paths:
        ...
        if filename in sequence_files_sizes_in_metamist.keys():
            ...
            if filesize == sequence_files_sizes_in_metamist.get(filename):
                ingested_and_moved_filepaths.append((path,sequence_files_paths_in_metamist.get(filename)))
...
def get_participant_sample_ids_for_project(project: str) -> defaultdict(list):
    """
    ...
    participants_samples = PAPI.get_external_participant_id_to_internal_sample_id(project=project)
...
def get_samples_for_project(project: str):
    ...
    """
    return SAPI.get_all_sample_id_map_by_internal(project=project)


def analyse_and_validate_participants_samples(participants: list[dict], participant_sample_ids: defaultdict[list], samples: dict[str,str]):
    """
    Report if any participants have multiple samples or no samples.
    Report if any samples have no participants.
    """
    # Get the external_participant_id : internal_participant_id mapping for all participants
    participant_eid_to_iid = {participant['external_id']: participant['id'] for participant in participants}

    # Report if any participants have more than one sample ID
    for participant_eid, sample_ids in participant_sample_ids.items():
        if len(sample_ids)>1:
            logging.info(f'Participant {participant_eid_to_iid[participant_eid]} (external ID: {participant_eid})' +
    ...
    participant_eid_set= set(participant['external_id'] for participant in participants)
    participant_eid_from_samples_set = set(participant_sample_ids.keys())
    logging.info(f'Participants with no samples: {len(participant_eid_set.difference(participant_eid_from_samples_set))}')

    # Compare the samples obtained from SampleApi vs ParticipantApi (papi)
    sample_id_set = set(samples.keys())
    sample_id_from_papi_set = set([item for sublist in participant_sample_ids.values() for item in sublist])
    logging.info(f'Samples with no participants: {sample_id_set.difference(sample_id_from_papi_set)}')


def get_sequences_for_samples(sequences: list[dict]):
    """
    Get all the sequence read locations for the sequences associated with the samples in the project
    Create mappings of {sequence_id: sample_id}, and {sequence_id: read_locations}
    """

    seq_id_sample_id = {}
    sequence_reads = defaultdict(list) # multiple reads per sequence is possible so use defaultdict(list)
            ...
            if isinstance(read, list):
                for inner_read in read:
                    if not isinstance(inner_read, dict):
                        ...
                        continue
                    sequence_reads[seq['id']].append((inner_read['location'],inner_read['size']))
            else:
...
def get_analysis_cram_paths_for_project_samples(project: str, samples: dict[str,str]):
    ...
    Returns a mapping of sample_id: cram_path
    """
    sample_ids = list(samples.keys())
    analyses = AAPI.query_analyses(
        AnalysisQueryModel(
            projects=[project, 'seqr'], # Many analysis objects hiding out in seqr project
    ...
    for analysis in analyses:
        if not validate_analysis_output(analysis['output']):
            logging.info(f'Analysis {analysis["id"]} invalid output path: {analysis["output"]}')
            print(f'Analysis {analysis["id"]} invalid output path: {analysis["output"]}')
            continue
        try:
...
def validate_analysis_output(output_path: str):
    """
    
    """
    return output_path.startswith('gs://') and output_path.endswith('cram')


def get_complete_and_incomplete_samples(samples: list[str], sample_cram_paths: dict[str,list[str]]):
    """
    ...
    buckets_subdirs = get_bucket_subdirs_to_search(sample_cram_paths.values())
    crams_in_bucket = get_files_in_buckets_subdirs(buckets_subdirs, 'cram')
...
def get_reads_to_delete(completed_samples: list[str], sequence_reads: dict[int,list[tuple[str,int]]], sample_id_seq_id_map: dict[str,list[int]]) -> dict[int,list[str]]:
    """
    Returns a list of sequence read files which can be safely deleted
    """
...
@click.command()
@click.option('--project', default='ravenscroft-rdstudy', help='Metamist project, used to filter samples')
...
def main(
    ...
    analyse_and_validate_participants_samples(participants_all, participant_sample_ids, samples_all)

    # Get all the sequences for the samples in the project and map them to their samples and reads
    sequences = SEQAPI.get_sequences_by_sample_ids(request_body= list(samples_all.keys()))
    ...
    sample_cram_paths = get_analysis_cram_paths_for_project_samples(project, samples_all)

    for sample, cram_path in sample_cram_paths.items():
        if not validate_analysis_output(cram_path):
            raise ValueError(f'{sample} cram path does not start with gs://: {cram_path}')

    # Identify samples with and without completed crams    
    sample_completion = get_complete_and_incomplete_samples(samples_all, sample_cram_paths)

    if sample_completion.get('incomplete'): 
        logging.info(f'Samples without CRAMs found: {sample_completion.get("incomplete")}')
    
    # Samples with completed crams can have their sequences deleted
    sequence_reads_to_delete = get_reads_to_delete(sample_completion.get('complete'), sequence_reads, sample_id_seq_id)

    # Check for uningested sequence data that may be hiding
    uningested_paths, moved_sequence_paths = check_for_uningested_sequences(project, sequence_reads)

    sequences_to_delete_file = f'/Users/edwfor/Code/sample-metadata/data_management_scripts/audit_results/seq_del_{project}.csv'
    with open(sequences_to_delete_file, 'w') as f:
        ...
        print(f'Wrote {i} reads to delete')
        i=0
        for seq,reads in moved_sequence_paths.items():
            for read in reads:
                i+=1
                writer.writerow([seq,read])
        print(f'Wrote {i} reads to delete which were ingested and moved')
        
    print(f'Wrote sequences to delete: {sequences_to_delete_file}')

    sequences_to_ingest_file = f'/Users/edwfor/Code/sample-metadata/data_management_scripts/audit_results/seq_ingest_{project}.csv'
    with open(sequences_to_ingest_file,'w') as f:
        ...
        for path in uningested_paths:
            ...
            i+=1
        print(f'Wrote {i} uningested sequences')

    print(f'Wrote sequences to ingest: {sequences_to_ingest_file}')
