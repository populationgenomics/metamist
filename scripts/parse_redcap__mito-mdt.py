""" Parses MitoMDT redcap dump ingest into metamist """
...
PROJECT = "mito-mdt"
UPLOAD_BUCKET_SEARCH_PATH = 'gs://cpg-mito-mdt-main-upload/'
...
def prepare_ped_row(individual_id, individual_metadata, family_metadata):
    ...
    # notes
    notes = individual_metadata['mito_clin_summary']
    if individual_metadata['mito_muscpres_nf']:
        notes += (
            '\n\nmuscpres_nf Reason:\n' + individual_metadata['mito_muscpres_nf']
        )
    if individual_metadata['mito_cnspres_nf']:
        notes += (
            '\n\ncnspres_nf:\n' + individual_metadata['mito_cnspres_nf']
        )
    if individual_metadata['mito_multisystem_nf']:
        notes += (
            '\n\nmultisystem_nf:\n' + individual_metadata['mito_multisystem_nf']
        )
    if individual_metadata['mito_metabolic_oth']:
        notes += (
            '\n\nmetabolic_oth :\n' + individual_metadata['mito_metabolic_oth']
        )
    if individual_metadata['mito_chem_nf']:
        notes += (
            '\n\nchem_nf :\n' + individual_metadata['mito_chem_nf']
        )
    if individual_metadata['mito_mdt_suggested_dx']:
        notes += (
            '\n\nMDT suggested_dx :\n' + individual_metadata['mito_mdt_suggested_dx']
        )
    if individual_metadata['mito_mdt_candidates']:
        notes += (
            '\n\nMDT candidates :\n' + individual_metadata['mito_mdt_candidates']
        )
...
def prepare_ind_row(individual_id, individual_metadata, family_metadata):
    ...
    hpo_present = [
        ...
        if '_hpo' in key and value and "HP:" in value
    ]
    return {
        'Family ID': family_metadata['fam_id'],
        'Individual ID': individual_id,
        ...
        'HPO Terms (absent)': "",
        'Individual Notes': "",
        'Consanguinity': individual_metadata['mito_consanguinity_yn'],
    }
...
async def main(redcap_csv: str, search_path: str, facility: str, dry_run: bool):
    """
    Parse a custom formatted CSV dump from the MitoMDT Redcap database
    ...
    # the mito MDT project
    assert (
        'MitoMDTParticipantDa-CPGMetadata' in redcap_csv
    ), "Is this a mito-mdt redcap csv?"
