...
class TestSubsetGenerator:  # pylint: disable=too-many-public-methods
    """Creates test subsets for a given dataset."""
    ...
    def __init__(
        ...
        self.families_n = families_n
        self.additional_families = additional_families
        self.additional_samples = additional_samples
        self.skip_ped = skip_ped
        self.seed = seed
    ...
    def find_families_samples_to_move(self, project: str, families_n: int, additional_families: set[str], additional_samples: set[str]) -> None:
        """
        Find samples to move from the original project to the test project.
        """
        additional_samples.update(
            self.get_sids_for_families(project, families_n, additional_families)
        )

    @staticmethod
    def find_all_sample_ids_in_project(project: str) -> set[str]:
        """
        Find all sample ids in the project.
        """
        logger.info(f'Querying all sample ids in {project}')
    ...
    def randomly_select_from_remaining_samples(self, all_sids: set[str], additional_samples: set[str], samples_n: int):
        """
        Choose n samples from all the samples in the project minus the specified additional samples.
        """
        # Cast to sorted list to maintain reproducibility with random sampling
        additional_samples.update(self.local_random.sample(sorted(list(all_sids - additional_samples)), samples_n,))

    @staticmethod
    def get_original_project_subset_data_samples(project: str, additional_samples: set[str]) -> list[dict]:
        """
        Get the data for the samples to be included in the test project.
        """
        ...
        return original_project_subset_data['project']['samples']  # pylint: disable=unsubscriptable-object

    @staticmethod
    def get_participant_data(samples: list[dict]) -> list:
        """
        Get the participant data for the samples to be included in the test project.
        """
        participant_data = []
    ...
    @staticmethod
    def get_participant_ids_from_participant_data(participant_data: list) -> list:
        """
        Get the list of participant ids from the participant data.
        """
        return [participant.get('id') for participant in participant_data]

    @staticmethod
    def get_target_project_name(project: str) -> str:
        """
    ...
    def get_existing_sg(
        self, existing_data: dict, sample_id: str, sg_type: str = None, sg_id: str = None
    ) -> dict | None:
        """
        ...
        if sample := self.get_existing_sample(existing_data, sample_id):
    ...
    def get_existing_assay(
        self, data: dict, sample_id: str, sg_id: str, original_assay: dict
    ) -> dict | None:
        """
        ...
        """
        if sg := self.get_existing_sg(existing_data=data, sample_id=sample_id, sg_id=sg_id):
    ...
    def get_existing_analysis(
        self, data: dict, sample_id: str, sg_id: str, analysis_type: str
    ) -> dict | None:
        """
    ...
    def transfer_samples_sgs_assays(
        ...
        for s in samples:
            ...
            existing_sample = self.get_existing_sample(existing_data, s['externalId'])
            if existing_sample:
            ...
            sample_upsert = SampleUpsert(
                ...
                meta=(self.copy_files_in_dict(s['meta'], project) or {}),
                participant_id=existing_pid,
                sequencing_groups=self.upsert_sequencing_groups(s, existing_data),
                id=existing_sid,
            )
    ...
    def upsert_assays(
        self, sg: dict, existing_sgid: str | None, existing_data: dict, sample_external_id
    ) -> list[AssayUpsert]:
        ...
        for assay in sg.get('assays'):
            ...
            if existing_sgid:
                _existing_assay = self.get_existing_assay(
                    existing_data,
                    sample_external_id,
                    existing_sgid,
                    assay,
    ...
    def transfer_analyses(
        self, samples: list[dict], existing_data: dict, target_project: str, project: str
    ):
        """
        ...
        for s in samples:
            for sg in s['sequencingGroups']:
                ...
                for analysis in sg['analyses']:
                    ...
                    if existing_sgid:
                        existing_analysis = self.get_existing_analysis(
                    ...
                    if existing_analysis_id:
                        am = AnalysisUpdateModel(
                            type=analysis['type'],
                            output=self.copy_files_in_dict(
                                analysis['output'],
                                project,
    ...
    def get_sids_for_families(
        self, project: str, families_n: int, additional_families: set[str]
    ) -> set[str]:
    ...
    def transfer_families(
        self, initial_project: str, target_project: str, internal_participant_ids: list[int]
    ) -> list[int]:
    ...
    def transfer_ped(
        self, initial_project: str, target_project: str, family_ids: list[int]
    ) -> dict[str, int]:
    ...
    def get_random_families(
        ...
        if sizes <= families_n:
            for _s, fams in distributed_by_size.items():
                ...
                returned_families.extend(self.local_random.sample(sorted(fams), n_pull))

        else:
            # we can't evenly distribute, so we'll just pull randomly
            returned_families = self.local_random.sample(sorted(families_within_threshold), families_n)

        return returned_families

    def copy_files_in_dict(self, d, dataset: str, sid_replacement: tuple[str, str] = None):
        """
        ...
        if isinstance(d, str) and d.startswith(f'gs://cpg-{dataset}-main'):
            ...
            for ext in extra_exts:
                if self.file_exists(old_path + ext) and not self.file_exists(new_path + ext):
                    cmd = f'gsutil cp {old_path + ext!r} {new_path + ext!r}'
                    logger.info(f'Copying extra file in metadata: {cmd}')
                    subprocess.run(cmd, check=False, shell=True)
            return new_path
        if isinstance(d, list):
            return [self.copy_files_in_dict(x, dataset) for x in d]
        if isinstance(d, dict):
            return {k: self.copy_files_in_dict(v, dataset) for k, v in d.items()}
        return d


@click.command(help='Create test subset for dataset')
@click.option(
    '--project',
    help='The metamist project/dataset to subset from',
)
@click.option('--samples-n', help='# Random Samples to copy', default=0)
@click.option('--families-n', help='# Random families to copy', default=0)
@click.option('--additional-family', help='Additional families to include', multiple=True, default=())
@click.option('--additional-sample', help='Additional samples to include', multiple=True, default=())
@click.option('--skip-ped', is_flag=True)
def main(
    project: str,
    samples_n: int,
    families_n: int,
    additional_family: tuple[str],
    additional_sample: tuple[str],
    skip_ped: bool = True,
):
    """
    ...
    if not any([additional_family, additional_sample, samples_n, families_n]):
        raise ValueError('Come on, what exactly are you asking for?')

    additional_samples = set(additional_sample)
    additional_families = set(additional_family)
    ...
    if families_n or additional_families:
        tsg.find_families_samples_to_move(project, families_n, additional_families, additional_samples)

    # 2. Get all sample IDs and their SG IDs in project.
    all_sids = tsg.find_all_sample_ids_in_project(project)

    # 3. Randomly select from the remaining samples
    tsg.randomly_select_from_remaining_samples(all_sids, additional_samples, samples_n)

    # 4. Query all the samples from the selected sgs
    samples = tsg.get_original_project_subset_data_samples(project, additional_samples)

    # 5. Pull Participant Data and IDs
    ...
    target_project = tsg.get_target_project_name(project)
    ...
    if skip_ped:
        ...
        upserted_participant_map = tsg.transfer_participants(
            target_project=target_project,
            participant_data=participant_data,
        )
    else:
        logger.info(f'Transferring {len(participant_data)} participants. ')
        family_ids = tsg.transfer_families(
            project, target_project, participant_internal_ids
        )
        upserted_participant_map = tsg.transfer_ped(project, target_project, family_ids)

    # Find what's already in the test project
    existing_data = query(EXISTING_DATA_QUERY, {'project': target_project})

    # Upsert samples, sequencing groups, and assays into the test project
    logger.info('Transferring samples, sequencing groups, and assays')
    tsg.transfer_samples_sgs_assays(
        samples, existing_data, upserted_participant_map, target_project, project
    )

    # Upsert analyses into the test project
    logger.info('Transferring analyses')
    tsg.transfer_analyses(samples, existing_data, target_project, project)
