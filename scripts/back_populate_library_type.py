...
async def update_sequencing_groups_async(
    sequencing_groups_to_update: list[dict], project: str
):
...
def main(
    project: str, sequencing_type: str, update_sequencing_groups: bool, dry_run: bool
):
    ...
    project_sg_assays: dict[str, dict] = query(
        _sg_assays_query,
    ...
    for sequencing_group, assays in sg_assays.items():
        ...
        if sg_meta_fields_to_update and update_sequencing_groups:
            sequencing_groups_to_update.append(
                {sequencing_group: sg_meta_fields_to_update}
            )
    ...
    if dry_run:
        ...
        if update_sequencing_groups:
            asyncio.run(
                update_sequencing_groups_async(sequencing_groups_to_update, project)
            )
            logging.info(
