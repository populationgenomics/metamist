...
def get_sample_data(project):
    return query(SAMPLE_QUERY, {'project': project})

def create_participant(sample_data: list, project_id: int, peid: str, sex: int):
    participant_upsert = [
        ParticipantUpsert(
            id=None,
            external_ids={'': peid},
...
def main(project: str, participant_meta: str, sample_meta: str):
    ...
    project_id = data_response['project']['id']
    ...
    participant_sample_count = defaultdict(int)
    metadata_map = {}
    ...
    participant_sex_map = {}
    participant_sample_map = defaultdict(list)
    ...
    for peid, sample_upserts in participant_sample_map.items():
        participant_upserts.append(
            create_participant(sample_upserts, project_id, peid, participant_sex_map[peid])
        )
