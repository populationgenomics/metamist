...
def get_analysis_types():
    ...
    analysis_types = query(ANALYSIS_TYPES_QUERY)
...
class GenericAuditor(AuditHelper):
    ...
    def get_participant_data_for_dataset(self) -> list[dict]:
        ...
        logging.getLogger().setLevel(logging.WARN)
        participant_query_results = query(
    ...
    def analyses_for_sgs_without_crams(self, sgs_without_crams: list[str]):
        ...
        logging.getLogger().setLevel(logging.WARN)
        sg_analyse_query_result = query(
            QUERY_SG_ANALYSES,
            {
                'dataset': self.dataset,
                'sgIds': sgs_without_crams,
                'analysisTypes': [t for t in get_analysis_types() if t != 'cram'],
            },
        )
        logging.getLogger().setLevel(logging.INFO)
    ...
    def check_for_uningested_or_moved_assays(  # pylint: disable=R0914
        self,
        bucket_name: str,
    ...
    def get_reads_to_delete_or_ingest(
        ...
        ) = self.check_for_uningested_or_moved_assays(
            bucket_name,
