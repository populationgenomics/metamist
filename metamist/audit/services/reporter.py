...
class Reporter:
    ...
    def __init__(
        ...
        audit_logger: BucketAuditLogger,
        ...
        Args:
            ...
            audit_logger: Audit logs instance
        """
        self.gcs_data = gcs_data
        self.audit_logger = audit_logger
        self.results_folder = timestamp or datetime.now().strftime('%Y-%m-%d_%H%M%S')
        self.output_dir = f'audit_results/{self.results_folder}'

    # Reading reports #
    def get_report_rows(self, report_path: Path) -> list[AuditReportEntry]:
        """Retrieve rows from the audit report CSV."""
        if not report_path.exists():
            self.audit_logger.error(f'Report file {report_path} does not exist.')
            return []
    ...
    def get_report_rows_from_name(self, name: str) -> list[AuditReportEntry]:
        ...
        report_path = to_path(
            f'gs://{self.gcs_data.analysis_bucket}/audit_results/{self.results_folder}/{name}.csv'
        )
    ...
    def write_audit_reports(
        ...
        if audit_result.unaligned_sequencing_groups:
            self.write_unaligned_sgs_report(
    ...
    def write_reviewed_files_report(
        ...
        """
        return self._write_csv_report(
            blob_path=f'audit_results/{self.results_folder}/reviewed_files.csv',
    ...
    def write_deleted_files_report(
        ...
        """
        if dry_run:
            blob_path = f'audit_results/{self.results_folder}/deleted_files_dry_run.csv'
        else:
            blob_path = f'audit_results/{self.results_folder}/deleted_files.csv'
        return self._write_csv_report(
            blob_path=blob_path,
    ...
    def write_unaligned_sgs_report(
        self,
        blob_path: str,
