...
class StorageClient:
    ...
    def list_blobs(
        ...
        for item in self.client.list_blobs(bucket, prefix=prefix or ''):
            ...
            files.append(
                FileMetadata(
                    filepath=file_path, filesize=blob.size, checksum=blob.crc32c
                )
            )
    ...
    def batch_get_metadata(self, paths: List[str]) -> List[FileMetadata]:
        """
    ...
    def find_files_in_prefixes(
        self, bucket_prefixes: Dict[str, List[str]], file_extensions: tuple[str, ...]
    ) -> Dict[str, List[str]]:
        """
    ...
    def extract_bucket_and_prefixes(self, paths: List[str]) -> Dict[str, List[str]]:
        """
