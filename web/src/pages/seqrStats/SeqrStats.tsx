...
const SeqrStats: React.FC = () => {
    ...
    return (
        <div>
            ...
            <SeqrProjectSelector projectNames={seqrProjectNames} projectIds={seqrProjectIds} onProjectSelected={handleProjectSelected}/>
            <select onChange={(e) => {
                setPageSize(parseInt(e.currentTarget.value))
                setPageNumber(0)
            }} value={pageSize}>
                {[1, 10, 50, 100].map(value => <option value={value} key={`setPageSize-${value}`}>{value}</option>)}
            </select>
        <Table>
            <thead>
                <tr>
        ...
        Page {pageNumber+1}
        {(((pageNumber + 1) * pageSize) < responses.length) && <button onClick={() => setPageNumber(pageNumber+1)}>Next page</button>}
        </div>
    );
};
