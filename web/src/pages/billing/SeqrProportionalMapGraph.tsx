                    ...
                    <g id={`${id}-stacked-areas`}>
                        ...
                        {stackedData.map((area, i) => (
                            <React.Fragment key={`bigArea-${i}`}>
                                {area.map((region, j) => {
                                    ...
                                    const colour = interpolateRainbow(i / selectedProjects.length)
                        ...
                        {stackedData.map((area, i) => {
                            ...
                            return (
                                <path
                                    key={`bigArea-${i}`}
                                    d={areaGenerator(area.slice(projectStart - 1, area.length + 1))}
                                    style={{
                                        stroke: selectedProjects[i] === hovered ? 'black' : 'none',
                                        strokeWidth: selectedProjects[i] === hovered ? 2 : 'none',
