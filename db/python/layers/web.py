...
class WebLayer(BaseLayer):
    ...
    async def get_projects_seqr_stats(
        ...
        """
        webprojectsdb = WebProjectsSeqrDb(self.connection)
        return await webprojectsdb.get_projects_seqr_stats(
            projects=projects, sequencing_types=sequencing_types
        )

    async def get_project_seqr_details(
        self,
        projects: list[int],
        sequencing_types: list[str],
    ) -> list[ProjectSeqrDetailsInternal]:
        """
        Get details for a list of projects
        """
        webprojectsdb = WebProjectsSeqrDb(self.connection)
        return await webprojectsdb.get_project_seqr_details(
            projects=projects, sequencing_types=sequencing_types
        )


class WebProjectsSeqrDb(DbBase):
    """Db layer for project seqr stats and details routes"""

    async def _projects_seqr_details_families_query(
...
ORDER BY
    ...
    async def _projects_seqr_stats_families_query(
        self, projects: list[int], sequencing_types: list[str]
    ):
        _query = """
SELECT
...
GROUP BY
    ...
    async def _projects_seqr_stats_participants_query(
        self, projects: list[int], sequencing_types: list[str]
    ):
        _query = """
SELECT
...
GROUP BY
    ...
    async def _projects_seqr_stats_samples_query(
        self, projects: list[int], sequencing_types: list[str]
    ):
        _query = """
SELECT
...
GROUP BY
    ...
    async def _projects_seqr_stats_sequencing_groups_query(
        self, projects: list[int], sequencing_types: list[str]
    ):
        _query = """
SELECT
...
GROUP BY
    ...
    async def _projects_seqr_stats_crams_query(
        self, projects: list[int], sequencing_types: list[str]
    ):
        _query = """
SELECT
...
WHERE
    ...
    async def _projects_seqr_stats_es_indices_query(
        self, projects: list[int], sequencing_types: list[str]
    ):
        _query = """
SELECT
...
AND
    a.project in :projects
        """
        latest_es_index_by_project_id_and_seq_type = await self.connection.fetch_all(
            _query,
            {
                'projects': projects,
                'sequencing_types': sequencing_types,
        ...
        return latest_es_index_by_project_id_and_seq_type

    async def _projects_seqr_stats_annotate_dataset_query(
        self, projects: list[int], sequencing_types: list[str]
    ):
        _query = """
SELECT
...
AND
    ...
    async def _projects_seqr_details_sequencing_groups_report_links(
        self, projects: list[int], sequencing_types: list[str]
    ):
...
WHERE
    ...
    async def get_projects_seqr_stats(
        ...
        (
            ...
            self._projects_seqr_stats_families_query(projects, sequencing_types),
            self._projects_seqr_stats_participants_query(projects, sequencing_types),
            self._projects_seqr_stats_samples_query(projects, sequencing_types),
            self._projects_seqr_stats_sequencing_groups_query(
                projects, sequencing_types
            ),
            self._projects_seqr_stats_crams_query(projects, sequencing_types),
            self._projects_seqr_stats_es_indices_query(projects, sequencing_types),
            self._projects_seqr_stats_annotate_dataset_query(
                projects, sequencing_types
            ),
        )
        ...
        for pid in projects:
            ...
            for sequencing_type in sequencing_types:
                ...
                latest_es_index_id = self.get_val_for_project_and_sequencing_type(
                    project.id, sequencing_type, 'id', latest_es_index
                )
                ...
                response.append(
                    ProjectSeqrStatsInternal(
                        ...
                        latest_es_index_id=latest_es_index_id,
                        total_sgs_in_latest_es_index=len(
                            await self._get_sequencing_groups_from_analysis_id(
                                latest_es_index_id
                            )
                        ),
                        latest_annotate_dataset_id=latest_annotate_dataset_id,
                        total_sgs_in_latest_annotate_dataset=len(
                            await self._get_sequencing_groups_from_analysis_id(
                                latest_annotate_dataset_id
                            )
                        ),
    ...
    async def get_project_seqr_details(
        ...
        (
            ...
            self._projects_seqr_details_families_query(projects, sequencing_types),
            self._projects_seqr_stats_crams_query(projects, sequencing_types),
            self._projects_seqr_stats_es_indices_query(projects, sequencing_types),
            self._projects_seqr_stats_annotate_dataset_query(
                projects, sequencing_types
            ),
            self._projects_seqr_details_sequencing_groups_report_links(
                projects, sequencing_types
            ),
        )
        ...
        for pid in projects:
            ...
            for sequencing_type in sequencing_types:
                ...
                sequencing_groups_in_latest_es_index = [
                ...
                for family_row in families:
                    ...
                    response.append(
                        ProjectSeqrDetailsInternal(
                            ...
                            in_latest_es_index=family_row['sequencing_group_id']
                            in sequencing_groups_in_latest_es_index,
